drop function public.search_businesses(text);
...
comment on function public.search_businesses(text, text) is
  'Busca negocios por relevancia y permite limitarlos a una categoría activa.';

revoke all on function public.search_businesses(text, text) from public;
grant execute on function public.search_businesses(text, text)
